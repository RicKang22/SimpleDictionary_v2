...
import com.android2016ncu.simpledictionary_v2.db.WordsSQLiteOpenHelper;
...
public class WordsAction {
     ...
     * 从数据库中查找查询的words
     * key 查找的值
     * words 若返回words的key为空，则说明数据库中没有该词
     */
    public Words getWordsFromSQLite(String key) {
        Words words = new Words();
        Cursor cursor = db.query(TABLE_WORDS, null, "keys=?", new String[]{key}, null, null, null);
        //数据库中有
        if (cursor.getCount() > 0) {
            LogUtil.d("测试", "数据库中有该单词");
            if (cursor.moveToFirst()) {
                do {
                    ...
                    words.setKey(cursor.getString(cursor.getColumnIndex("keys")));
